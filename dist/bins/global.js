...
function getCommand(reqCom, params) {
    ...
    else if (cmd && reqCom && reqCom != libName) return {isAutoComplete:!isDirectFind&&!params.includes("-y")&&!params.includes("-f"), cmdName:cmd[0], cmd:[join(dirname(process.argv[1]), cmd[1]), ...params.filter(x=>!x.includes("-y")&&!x.includes("-f"))].filter(Boolean)}
}
...
if (command) {
    if (command.isAutoComplete) {
        ...
        cli.question("The command was autocompleted to '"+libName+"-"+command.cmdName+"', continue [Y/N]? ", value=>{
