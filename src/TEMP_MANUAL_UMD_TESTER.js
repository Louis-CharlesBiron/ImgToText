/*
  ...
  What could work:
  1. get CDEJS minified
  2. remove its umd wrapper, but keep the its $UMDCJSClasses
  3. put bare CDEJS into the umd wrapper with ImgToText code
  4. return both the {CDEJS} and {ImgToText} classes
  ...
  like:
    ...
    if (typeof define === "function" && define.amd) define([], factory)
        ...
        const {CDE, ImgToText} = factory()
        window.CDE = CDE
        window.ImgToText = ImgToText
    }
    else {
        const {CDE, ImgToText} = factory()
        this.CDE = CDE
...
(function(factory){
    ...
    else if (typeof window !== "undefined") window.ImgToText = factory()
    else this.ImgToText = factory()
  })(function(){
    ...
    // CanvasDotEffect UMD - v1.1.7
    (function(factory){if(typeof define==="function"&&define.amd)define([],factory);else if(typeof module==="object"&&module.exports)module.exports=factory();else if(typeof window!=="undefined")window.CDE=factory();else this.CDE=factory()})(function(){
      
      // --- $minifiedCode (from CDE)
      "use strict";class CDEUtils{static DEFAULT_ACCEPTABLE_DIFFERENCE=1e-7;static CIRC=2*Math.PI;static TO_DEGREES=Math.PI/180;set connections(c){return this._connections=c}set cachedPath(path){this._cachedPath=path}};

      // --- $UMDCJSClasses (from CDE)
      return{CDEUtils,FPSCounter,CanvasUtils,Color,_HasColor,GridAssets,TypingDevice,Mouse,Render,TextStyles,RenderStyles,Canvas,Anim,_BaseObj,AudioDisplay,ImageDisplay,TextDisplay,_DynamicColor,Pattern,_Obj,Shape,Gradient,FilledShape,Grid,Dot,}
    ...
    return {
      ImgToText:ImageToTextConverter, 
      CDE:{CDEUtils,FPSCounter,CanvasUtils,Color,_HasColor,GridAssets,TypingDevice,Mouse,Render,TextStyles,RenderStyles,Canvas,Anim,_BaseObj,AudioDisplay,ImageDisplay,TextDisplay,_DynamicColor,Pattern,_Obj,Shape,Gradient,FilledShape,Grid,Dot}
    }
...
if(typeof define==="function"&&define.amd)define([],factory);else if(typeof module==="object"&&module.exports)module.exports=factory();else if(typeof window!=="undefined"){const{CDE,ImgToText}=factory();window.CDE=CDE;window.ImgToText=ImgToText}else{const{CDE,ImgToText}=factory();this.CDE=CDE;this.ImgToText=ImgToText}